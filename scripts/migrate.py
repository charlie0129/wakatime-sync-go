...
def parse_args():
    parser = argparse.ArgumentParser(description='Migrate WakaTime data from MySQL to SQLite')
    ...
    parser.add_argument('--sqlite-path', default='wakatime.db', help='SQLite database path')
...
def create_sqlite_schema(sqlite_conn: sqlite3.Connection):
    ...
    ''')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_project_durations_day ON project_durations(day)')
...
def migrate_durations(mysql_conn, sqlite_conn: sqlite3.Connection):
    ...
    for row in mysql_cursor:
        ...
        if start_time:
            ...
            # Handle dependencies (might be JSON)
            deps = row.get('dependencies')
            if deps and isinstance(deps, (list, dict)):
